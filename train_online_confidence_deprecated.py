...
class OnlineEnvironmentManager:
    """Manages growing contexts for online environments with model-based sampling and confidence-based targets."""

    def __init__(self, envs, env_type, horizon, config, model=None, confidence_start=0.3, **kwargs):
        self.envs = envs
        self.env_type = env_type
        self.horizon = horizon
        self.config = config
        self.model = model
        self.confidence_start = confidence_start
        self.kwargs = kwargs
        self.reset_contexts()
    ...
    def step_and_learn(self, optimizer, loss_fn, action_dim, horizon, current_iteration, debug=False):
        ...
        # Calculate confidence schedule: start_value * (1 - t/H) + 1 * t/H
        confidence = self.confidence_start * (1 - current_iteration / horizon) + 1.0 * (current_iteration / horizon)
        ...
        if debug and len(env_data) > 0:
            ...
            for debug_idx in range(num_debug_envs):
                ...
                if self.env_type in ['bandit', 'linear_bandit']:
                    ...
                    prob_str = ", ".join([f"A{i}:{pred_probs[i].item():.3f}" for i in range(len(pred_probs))])
                print(f"Batch: Env {env_idx}: Context={data['context_size']}, Confidence={confidence:.3f}, Probs=[{prob_str}], SampledAction={sampled_action_idx}, OptimalAction={optimal_action_idx}, Match={sampling_match}, Reward={data['reward']:.3f}, Loss={total_loss:.4f}, BatchSize={batch_size}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--confidence_start', type=float, default=0.3, help='Starting confidence value for target interpolation')
    ...
    model_config = {
        ...
        'dim': dim,
        'seed': seed,
        'confidence_start': confidence_start,  # Add confidence to model config
    }
    ...
    # Update filename to include confidence parameter
    filename = filename.replace('.pt', f'_conf{confidence_start}.pt')
    
    log_filename = f'figs/loss/{filename}_online_confidence_logs.txt'
    ...
    # Create online environment manager with confidence
    if env == 'miniworld':
        env_manager = OnlineEnvironmentManager(
            [gym_env] * len(train_env_ids), env, horizon, config, model,
            confidence_start=confidence_start, transform=transform, target_shape=target_shape, env_ids=train_env_ids
        )
    elif env == 'linear_bandit':
        env_manager = OnlineEnvironmentManager(
            train_envs, env, horizon, config, model, confidence_start=confidence_start, arms=arms
        )
    else:
        env_manager = OnlineEnvironmentManager(
            train_envs, env, horizon, config, model, confidence_start=confidence_start
        )

    printw(f"Starting online training with confidence for {env}")
